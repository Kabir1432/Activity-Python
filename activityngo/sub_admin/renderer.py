...
__all__ = ["SubAdminRenderer"]


class SubAdminRenderer(JSONRenderer):
    def render(self, data, accepted_media_type=None, renderer_context=None):
            ...
            try:
                ...
                if "user_details" in error_dict and isinstance(error_dict[1], dict):
                    ...
                    if errors.find("This field is required.") != -1:
                        errors = f"{next(iter(error_dict[1]))} field required in user_details."
                    else:
                        errors = errors.replace(".", "") + " in user_details."
                else:
                    error_dict = next(iter((data.items())))
        ...
        return super(SubAdminRenderer, self).render(
